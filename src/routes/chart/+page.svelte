...
let x: string, y: string, dotInfo: any, lines: string[], xVals: number[] = [], yVals: number[] = [], points: { x: number; y: number; color: number }[] = [], subsets: any[] = [], colorVals: number[] = [];
    
    // For a single set of data
    if (!('data' in data2[0])) {
      x = Object.keys(data2[0])[0];
      y = Object.keys(data2[0])[1];
    ...
    // For data with subsets (NOTE: expects 'id' and 'data' keys)
      ...
      x = Object.keys(data2[0]?.data2[0])[0];
      y = Object.keys(data2[0]?.data2[0])[1];
